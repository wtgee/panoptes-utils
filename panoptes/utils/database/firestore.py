...
class PanFireStoreDB(AbstractPanDB):
    ...
    def insert_current(self, collection, obj, obj_id=None, store_permanently=True):
        if store_permanently:
            document_info = self.insert(collection, obj, obj_id=obj_id)
    ...
    def insert(self, collection, obj, obj_id=None):
        ...
        Args:
            ...
            obj_id (str, optional): The object id to use, if None then one is
        ...
        try:
            obj = self.create_storage_obj(collection, obj, obj_id=obj_id)

            # Insert record into db
            collection = getattr(self, collection)

            return collection.add(obj, obj_id=obj_id)
    ...
    def get_current(self, collection):
        ...
        if obj is None:
            collection = getattr(self, collection)
            collection.order_by('date').limit(1).get().to_dict()
